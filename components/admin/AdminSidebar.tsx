...
export default function AdminSidebar() {
  ...
  return (
    <>
      ...
      {/* ── MOBILE BOTTOM NAV ───────────────────────────────── */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0d1e3a] border-t border-gray-800 flex items-center justify-around px-1 h-16 safe-area-inset-bottom">
        {navItems.map((item) => {
          const active = isActive(item.href);
          return (
            <Link
